...
def load_to_postgres(file_path, table_name='clinical_summaries'):
    ...
    engine = create_engine(f"postgresql+psycopg2://{db_params['user']}:{db_params['password']}@{db_params['host']}:{db_params['port']}/{db_params['database']}")
    with engine.connect() as conn:
        conn.execution_options(autocommit=True).execute("SELECT 1")
...
# Charger les documents du bucket
...
def create_vector_store():
    ...
    bucket_documents, bucket_metadatas = load_documents_from_bucket()
    
    embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")
    batch_size = 100
    texts = df['combined_text'].tolist() + bucket_documents
    metadatas = df.to_dict('records') + bucket_metadatas
    vectorstore = None
